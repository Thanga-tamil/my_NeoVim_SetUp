...
-- Map <leader>c to toggle comments for multiple lines in Visual Block mode
vim.api.nvim_set_keymap('x', '<leader>c', ':<C-u>lua ToggleComment()<CR>', { noremap = true, silent = true })
...
function ToggleComment()
    ...
    for _, line in ipairs(lines) do
        ...
        or ((filetype == 'javascript' or filetype == 'html' or filetype == 'java') and not is_commented(line, "//")) then
            commented = false
            break
        end
    end
    ...
    for i, line in ipairs(lines) do
        ...
        elseif (filetype == 'javascript' or filetype == 'html' or filetype == 'java') then
            if commented then
                lines[i] = line:gsub("^%s*//%s*", "")
            else
                lines[i] = "// " .. line
